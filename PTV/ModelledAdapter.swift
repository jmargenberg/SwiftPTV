...
public class ModelledAdapter: Adapter {
    ...
    public func getRouteTypes(failure: @escaping Adapter.FailureHandler, completion: @escaping (_ routeTypes: [RouteType]) -> ()) {
    ...
    private func getRoutes(params: [String: String], failure: @escaping Adapter.FailureHandler, completion: @escaping (_ routes: [Route]) -> ()) {
    ...
    public func getRoutes(failure: @escaping Adapter.FailureHandler, completion: @escaping (_ routes: [Route]) -> ()) {
        self.call(apiName: "routes", searchString: "", params: nil, decodeTo: RoutesResponse.self, failure: failure) { (routesResponse) in
            completion(routesResponse.routes)
        }
    }
    
    public func getRoutes(forRouteTypeID routeTypeID: Int, failure: @escaping Adapter.FailureHandler, completion: @escaping (_ routes: [Route]) -> ()) {
        self.getRoutes(failure: failure) { (routes) in
            completion(routes.filter({ $0.route_type == routeTypeID }))
        }
    }
    
    public func getRoutes(forRouteTypeIDs routeTypeIDs: [Int], failure: @escaping Adapter.FailureHandler, completion: @escaping (_ routes: [Route]) -> ()) {
    ...
    public func getStops(nearLocation location: CLLocationCoordinate2D, forRouteTypes routeTypes: [Int] = [CachingAdapter.TRAIN_ROUTE_TYPE], maxResults: Int = 1000, maxDistance: Int = 5000, failure: @escaping Adapter.FailureHandler, completion: @escaping (([StopGeosearch]) -> ())) {
    ...
    public func getStops(onRoute routeId: Int, forRouteType routeTypeId: Int = CachingAdapter.TRAIN_ROUTE_TYPE, failure: @escaping Adapter.FailureHandler, completion: @escaping (([StopOnRoute]) -> ())) {
    ...
    public func getStopDetails(forStop stopId: Int, forRouteType routeTypeId: Int = CachingAdapter.TRAIN_ROUTE_TYPE, failure: @escaping Adapter.FailureHandler, completion: @escaping ((StopDetails) -> ())) {
    ...
    public func getStoppingPattern(forRun runId: Int, withRouteType routeTypeId: Int, failure: @escaping Adapter.FailureHandler, completion: @escaping (StoppingPattern) -> ()) {
